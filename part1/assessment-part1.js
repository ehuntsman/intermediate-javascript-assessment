...
String.prototype.grammarPolice = function(){
  ...
  for(var i = 0; i < result.length; i++){
    var shiny = result[i].toLowerCase();
    shiny = shiny.substring(1);
    result[i] = result[i].charAt(0).toUpperCase();
    result[i] = result[i] + shiny;
    var tempy = result[i].split("-");
    if(tempy.length > 1){
      for(var j = 0; j < tempy.length; j++){
        var daleks = tempy[j].substring(1);
        tempy[j] = tempy[j].charAt(0).toUpperCase();
        tempy[j] = tempy[j] + daleks;
      }
      result[i] = tempy.join("-");
    }
  }
